    ...
    public void crossValidation() {
        ...
        for (int f = 0; f < fold; f++) {
            try {
                ...
                Classifier c = (Classifier) Class.forName("com.telecom.naivebayes." + clsName).newInstance();
              //  Classifier c1 = (Classifier) Class.forName("com.telecom.naivebayes." + clsName).newInstance();
                c.train(isCategory, trainFeatures, trainLabels);
               // String json = c1.trainalldata(isCategory, trainFeatures,trainLabels);
                ...
                for (int j = 0; j < testLabels.length; j++) {
                   ...
                   // a.add(prediction);
                   // double prediction1 = c1.predictvalue(testFeatures[j],json);
                    //b.add(prediction1);
                    if (isClassification) {
