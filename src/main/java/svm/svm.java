...
class Cache {
	...
	l = l_;
	size = size_;
	head = new head_t[l];//head[i]��headʲô��ϵ
	for(int i=0;i<l;i++) head[i] = new head_t();//head_t()ʲô��˼
	size /= 4;
    ...
    private void lru_delete(head_t h)//ɾ��
    {
	...
	if(more > 0)
		...
		if(h.data != null) System.arraycopy(h.data,0,new_data,0,h.len);//��ָ��Դ����������ָ��λ�ÿ�ʼ���Ƶ�Ŀ�������ָ��λ��
		h.data = new_data;
		size -= more;
...
//���ݼ�������ǩ������C���ݴ��ʺ�ȡ��ǰ����ѭ��������w���ǲ�����ֵԽ��߽�Խ���� 
//C����ͷ�ϵ���������ĳ��x������ĳһ�࣬������ƫ���˸��࣬�ܵ��߽��Ϻ���������ĵط�ȥ�ˣ�CԽ�����Խ�����������㣬�߽�ͻ���С
//kesi������ɢ����
class Solver {
    int active_size;
    byte[] y;
    ...
    Kernel Q;
    double eps; //�����
    double Cp,Cn;//���ò�ͬ��Cp��Cn��Ϊ�˴������ݵĲ�ƽ�⣬�ͷ�����
    double[] b;
    int[] active_set;
    double[] G_bar;		// gradient, if we treat free variables as 0
    int l;
    boolean unshrinked;	// XXX//���ض�Ӧ��������C��
	
    static final double INF = java.lang.Double.POSITIVE_INFINITY;//һ����������double���������
    ...
    void swap_index(int i, int j)//��������i��j�����ݣ�����������ڴ�ĵ�ַ   
    {
	Q.swap_index(i,j);
	do {byte _=y[i]; y[i]=y[j]; y[j]=_;} while(false);//���������
	do {double _=G[i]; G[i]=G[j]; G[j]=_;} while(false);
	do {byte _=alpha_status[i]; alpha_status[i]=alpha_status[j]; alpha_status[j]=_;} while(false);//����µ�alpha
	...
	for(i=0;i<active_size;i++)
	    if(is_free(i))
		{
		    float[] Q_i = Q.get_Q(i,l);//��column��len��
...
class SVC_Q extends Kernel
{
	...
	y = (byte[])y_.clone();
	cache = new Cache(prob.l,(int)(param.cache_size*(1<<20)));//��λ���ƣ����� 2^20
    }
...
public class svm {
    ...
    public static void svm_save_model(DataOutput fp, svm_model model) throws IOException
    {
	...
	svm_model model = svm_load_model(fp);
	fp.close();
	return model;
    }
